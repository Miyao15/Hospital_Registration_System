...
const fetchLandingData = async () => {
  try {
    ...
    if (results[1].status === 'fulfilled') dentists.value = results[1].value.data;
    if (results[2].status === 'fulfilled') specialties.value = results[2].value.data;

    // 打印失败的详情方便调试
    results.forEach((res, i) => {
      if (res.status === 'rejected') console.error(`接口 ${i} 加载失败:`, res.reason);
    });
  } catch (error) {
